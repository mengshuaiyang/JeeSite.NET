...
function showAddChild(parent) { editItem.value = null; form.parentCode = parent.areaCode; form.areaName = ''; form.areaType = 'district'; modalOpen.value = true; }
...
async function handleDelete(code) { await areaApi.delete(code); message.success('删除成功'); loadData(); }
onMounted(loadData);
    ...
    if (column.key === 'action') {
        ...
        __VLS_asFunctionalElement(__VLS_intrinsicElements.a, __VLS_intrinsicElements.a)({
            ...{ onClick: (...[$event]) => {
                    if (!(column.key === 'action'))
                        return;
                    __VLS_ctx.handleDelete(record.areaCode);
                } },
        });
        var __VLS_20;
    }
