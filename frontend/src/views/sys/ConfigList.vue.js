...
const form = reactive({ configKey: '', configName: '', configValue: '', isSysBool: '0' });
async function loadData() { loading.value = true; const r = await configApi.list({ pageNo: data.pageNo, pageSize: data.pageSize, entity: {} }); if (r.data) {
...
function showEdit(r) { isEdit.value = true; modalTitle.value = '编辑参数'; form.configKey = r.configKey; form.configName = r.configName; form.configValue = r.configValue || ''; form.isSysBool = r.isSys || '0'; modalVisible.value = true; }
async function save() { await configApi.save({ configKey: isEdit.value ? form.configKey : form.configKey || undefined, configName: form.configName, configValue: form.configValue, isSys: form.isSysBool }); message.success('保存成功'); modalVisible.value = false; loadData(); }
async function handleDelete(k) { await configApi.delete(k); message.success('删除成功'); loadData(); }
onMounted(loadData);
    ...
    if (column.key === 'action') {
        ...
        __VLS_asFunctionalElement(__VLS_intrinsicElements.a, __VLS_intrinsicElements.a)({
            ...{ onClick: (...[$event]) => {
                    if (!(column.key === 'action'))
                        return;
                    __VLS_ctx.handleDelete(record.configKey);
                } },
        });
        var __VLS_20;
    }
...
const __VLS_38 = __VLS_asFunctionalComponent(__VLS_37, new __VLS_37({
    value: (__VLS_ctx.form.configKey),
...
const __VLS_46 = __VLS_asFunctionalComponent(__VLS_45, new __VLS_45({
    value: (__VLS_ctx.form.configName),
}));
...
const __VLS_54 = __VLS_asFunctionalComponent(__VLS_53, new __VLS_53({
    value: (__VLS_ctx.form.configValue),
}));
...
const __VLS_62 = __VLS_asFunctionalComponent(__VLS_61, new __VLS_61({
    checked: (__VLS_ctx.form.isSysBool),
    checkedValue: "1",
    unCheckedValue: "0",
}));
