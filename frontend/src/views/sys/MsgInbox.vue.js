...
async function loadData() {
    loading.value = true;
    const res = await msgApi.inbox({ pageNo: data.pageNo, pageSize: data.pageSize, entity: { msgTitle: query.msgTitle } });
    if (res.data) {
...
function rowClickHandler(record) {
    return { onClick: async () => { if (record.status === '0') {
            await msgApi.markRead(record.msgId);
...
const __VLS_14 = __VLS_asFunctionalComponent(__VLS_13, new __VLS_13({
    value: (__VLS_ctx.query.msgTitle),
    placeholder: "搜索",
}));
...
const __VLS_30 = __VLS_asFunctionalComponent(__VLS_29, new __VLS_29({
    ...
    rowKey: "msgId",
    customRow: (__VLS_ctx.rowClickHandler),
...
__VLS_32.slots.default;
{
    ...
    if (column.key === 'status') {
        ...
        const __VLS_34 = __VLS_asFunctionalComponent(__VLS_33, new __VLS_33({
            color: (record.status === '0' ? 'blue' : 'default'),
        }));
