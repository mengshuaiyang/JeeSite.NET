...
async function loadData() {
    loading.value = true;
    const res = await roleApi.list({ pageNo: data.pageNo, pageSize: data.pageSize, entity: { roleName: query.roleName } });
    if (res.data) {
...
function showEdit(row) { editItem.value = row; form.roleName = row.roleName; modalOpen.value = true; }
...
async function handleDelete(code) { await roleApi.delete(code); message.success('删除成功'); loadData(); }
onMounted(loadData);
...
const __VLS_14 = __VLS_asFunctionalComponent(__VLS_13, new __VLS_13({
    value: (__VLS_ctx.query.roleName),
    placeholder: "搜索",
}));
    ...
    if (column.key === 'action') {
        ...
        __VLS_asFunctionalElement(__VLS_intrinsicElements.a, __VLS_intrinsicElements.a)({
            ...{ onClick: (...[$event]) => {
                    if (!(column.key === 'action'))
                        return;
                    __VLS_ctx.handleDelete(record.roleCode);
                } },
        });
        var __VLS_48;
    }
...
const __VLS_50 = __VLS_asFunctionalComponent(__VLS_49, new __VLS_49({
    ...
    title: (__VLS_ctx.editItem ? '编辑角色' : '新增角色'),
