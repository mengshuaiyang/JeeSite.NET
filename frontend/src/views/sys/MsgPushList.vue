    ...
    <a-table :dataSource="data.list" :columns="columns" :loading="loading" rowKey="id"
      ...
      <template #bodyCell="{ record, column }">
        <template v-if="column.key==='pushStatus'">
          <a-tag :color="record.pushStatus==='success'?'green':'orange'">{{ {pending:'待推送',success:'已成功',fail:'失败',retrying:'重试中'}[record.pushStatus] || record.pushStatus }}</a-tag>
        </template>
...
async function loadData() {
  loading.value = true
  try {
    const res = await post('/sys/msg/push/list', { pageNo: data.pageNo, pageSize: data.pageSize })
...
async function handleSend() {
  sending.value = true
  try {
    const res = await post('/sys/msg/push/send', { ...form })
    if (res.code === 200) { message.success('发送成功'); sendOpen.value = false; loadData() }
    else message.error(res.message)
  } finally { sending.value = false }
}

async function retryPush(record: any) {
  const res = await post('/sys/msg/push/retry', { id: record.id })
