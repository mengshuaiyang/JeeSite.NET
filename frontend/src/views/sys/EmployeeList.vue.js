...
async function loadData() {
    loading.value = true;
    const res = await employeeApi.list({ pageNo: data.pageNo, pageSize: data.pageSize, entity: { empName: query.empName } });
    if (res.data) {
...
async function handleDelete(code) { await employeeApi.delete(code); message.success('删除成功'); loadData(); }
onMounted(loadData);
    ...
    if (column.key === 'action') {
        ...
        __VLS_asFunctionalElement(__VLS_intrinsicElements.a, __VLS_intrinsicElements.a)({
            ...{ onClick: (...[$event]) => {
                    if (!(column.key === 'action'))
                        return;
                    __VLS_ctx.handleDelete(record.empCode);
                } },
        });
        var __VLS_48;
    }
...
const __VLS_50 = __VLS_asFunctionalComponent(__VLS_49, new __VLS_49({
    ...
    title: (__VLS_ctx.editItem ? '编辑员工' : '新增员工'),
