...
const api = { tree: () => get('/test/tree/list'), save: (d) => post('/test/tree/save', d), delete: (c) => post('/test/tree/delete', { treeCode: c }) };
...
function showAddChild(parent) { editItem.value = null; form.parentCode = parent.treeCode; form.treeName = ''; modalOpen.value = true; }
...
async function handleDelete(code) { await api.delete(code); message.success('删除成功'); loadData(); }
onMounted(loadData);
    ...
    if (column.key === 'action') {
        ...
        __VLS_asFunctionalElement(__VLS_intrinsicElements.a, __VLS_intrinsicElements.a)({
            ...{ onClick: (...[$event]) => {
                    if (!(column.key === 'action'))
                        return;
                    __VLS_ctx.handleDelete(record.treeCode);
                } },
        });
        var __VLS_20;
    }
