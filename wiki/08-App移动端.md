...
- [一、模块概述](#一、模块概述)
  - [1.1 模块定位](#11-模块定位)
  - [1.2 技术依赖](#12-技术依赖)
  - [1.3 典型调用流程](#13-典型调用流程)
- [二、核心实体](#二、核心实体)
  - [2.1 AppComment（意见反馈）](#21-appcomment（意见反馈）)
  - [2.2 AppUpgrade（版本管理）](#22-appupgrade（版本管理）)
  - [2.3 AppUser（可选扩展）](#23-appuser（可选扩展）)
- [三、核心服务](#三、核心服务)
  - [3.1 AppCommentService](#31-appcommentservice)
  - [3.2 AppUpgradeService](#32-appupgradeservice)
- [四、控制器与 API](#四、控制器与-api)
  - [4.1 AppController](#41-appcontroller)
    - [4.1.1 移动端（匿名/用户）](#411-移动端（匿名-用户）)
    - [4.1.2 管理后台（需要权限）](#412-管理后台（需要权限）)
  - [4.2 请求 / 响应示例](#42-请求-响应示例)
    - [提交反馈（POST /api/v1/app/comment/submit）](#提交反馈（post-api-v1-app-comment-submit）)
    - [检查升级（GET /api/v1/app/upgrade/check?platform=Android&versionCode=100）](#检查升级（get-api-v1-app-upgrade-checkplatform=android&versioncode=100）)
- [五、前端页面（管理后台）](#五、前端页面（管理后台）)
- [六、APP 端集成示例](#六、app-端集成示例)
  - [6.1 启动时检查升级（伪代码）](#61-启动时检查升级（伪代码）)
  - [6.2 意见反馈表单](#62-意见反馈表单)
  - [6.3 DeviceInfo 推荐字段](#63-deviceinfo-推荐字段)
- [七、扩展方向](#七、扩展方向)
  - [7.1 AppUser 账号体系（推荐）](#71-appuser-账号体系（推荐）)
  - [7.2 消息推送](#72-消息推送)
  - [7.3 Banner / 启动广告](#73-banner-启动广告)
  - [7.4 支付接口封装](#74-支付接口封装)
  - [7.5 统计分析](#75-统计分析)
  - [7.6 其他常见能力](#76-其他常见能力)
