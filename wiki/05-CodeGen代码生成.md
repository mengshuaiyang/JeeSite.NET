...
- [一、模块概述](#一、模块概述)
  - [1.1 模块定位](#11-模块定位)
  - [1.2 生成内容清单](#12-生成内容清单)
  - [1.3 技术依赖](#13-技术依赖)
- [二、核心实体](#二、核心实体)
  - [2.1 GenTable（表配置）](#21-gentable（表配置）)
  - [2.2 GenTableColumn（列配置）](#22-gentablecolumn（列配置）)
  - [2.3 DTO 概览](#23-dto-概览)
- [三、核心服务与流程](#三、核心服务与流程)
  - [3.1 GenTableService（表管理）](#31-gentableservice（表管理）)
    - [主要方法](#主要方法)
    - [导入流程](#导入流程)
  - [3.2 CodeGenService（代码生成）](#32-codegenservice（代码生成）)
  - [3.3 CodeGenTemplates（模板管理）](#33-codegentemplates（模板管理）)
    - [模板可用变量](#模板可用变量)
  - [3.4 DbIntrospectionProvider（数据库自省）](#34-dbintrospectionprovider（数据库自省）)
- [四、控制器与 API](#四、控制器与-api)
  - [4.1 请求体示例：ImportAsync](#41-请求体示例：importasync)
  - [4.2 请求体示例：SaveAsync](#42-请求体示例：saveasync)
- [五、使用步骤](#五、使用步骤)
- [六、表设计规范](#六、表设计规范)
  - [6.1 命名](#61-命名)
  - [6.2 审计字段（推荐）](#62-审计字段（推荐）)
  - [6.3 树形表（IsTree = true）](#63-树形表（istree-=-true）)
  - [6.4 注释](#64-注释)
  - [6.5 示例建表 SQL（SqlServer）](#65-示例建表-sql（sqlserver）)
- [七、自定义模板扩展](#七、自定义模板扩展)
  - [7.1 修改默认模板](#71-修改默认模板)
  - [7.2 常用 Scriban 写法](#72-常用-scriban-写法)
  - [7.3 新增生成文件](#73-新增生成文件)
  - [7.4 常见变量速查](#74-常见变量速查)
- [八、代码生成后的步骤清单](#八、代码生成后的步骤清单)
  - [8.1 常见问题处理](#81-常见问题处理)
- [九、模块主要文件结构](#九、模块主要文件结构)
